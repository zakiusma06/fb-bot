...
from creative_hunt_sheet import (
    ...
    count_all_creatives,
    count_empty_slots,
    delete_creative_slot,
    send_back_to_pending,
    EXTRA_CREATIVE_COLS,
    APPROVED_TAB,
)
...
def _media_type_keyboard(product_idx: int, sku: str = "") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("⬅️ Back to Pending", callback_data=f"ch_back_pending:{sku}"),
        ],
    ])
...
async def _show_product_prompt(bot, session: dict):
    ...
    saved_creatives = get_existing_creatives(product)
    total_saved     = len(saved_creatives)
    ...
    text = (
        ...
        + shop_line
        + ("\n" + creatives_lines if creatives_lines else "")
        + f"\n📦 Open creative slots: <b>{empty_slots}</b>\n\n"
        "Which type of creatives should the bot look for?"
    )
...
async def _wait_and_show(bot, session: dict) -> None:
    ...
    waited       = 0
    search_ended = False  # True = scraper finished cleanly, False = real timeout
    ...
    if ad is None:
        ...
        if num_creatives > 0:
            msg = (
                f"✅ Search complete — no new creatives found.\n<b>{num_creatives}</b> creative(s) already saved."
                if search_ended else
                f"⏱ Search timed out. <b>{num_creatives}</b> creative(s) saved so far."
            )
            await bot.send_message(chat_id=session["chat_id"], text=msg, parse_mode="HTML", reply_markup=_approval_confirm_keyboard(sku, num_creatives))
        else:
            msg = (
                f"✅ Search complete — no creatives found for <b>{_esc(product_name)}</b>.\nMoving to next product…"
                if search_ended else
                f"⏱ Search timed out for <b>{_esc(product_name)}</b>.\nMoving to next product…"
            )
            await bot.send_message(chat_id=session["chat_id"], text=msg, parse_mode="HTML")
            await _advance_product(bot, session)
        return
...
async def cmd_setcookies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🍪 <b>Update Facebook Cookies</b>\n\nPaste your cookies JSON array (from Cookie-Editor) and send it.",
        parse_mode="HTML",
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_cookies"):
        ...
        try:
            ...
            await update.message.reply_text(f"✅ {len(pw)} cookies saved. New session active.", reply_markup=MAIN_MENU)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── BACK TO PENDING ──────────────────────────────────────────────────────
    if data.startswith("ch_back_pending:"):
        back_sku = data[len("ch_back_pending:"):]
        session  = _sessions.get(user_id)
        product  = {}
        if session and session.get("products"):
            idx = session.get("product_idx", 0)
            prods = session["products"]
            if idx < len(prods):
                product = prods[idx]
        url_product = str(product.get("URL PRODUCT", "")).strip()

        await query.message.reply_text("⏳ Sending back to Pending…")
        loop = asyncio.get_running_loop()

        ok = await loop.run_in_executor(
            None, lambda: send_back_to_pending(back_sku, APPROVED_TAB)
        )
        if ok:
            # Try to delete Shopify product
            if url_product:
                try:
                    import shopify_cache as _sc
                    _sc.load_from_file()
                    shopify_pid = None
                    for pid, cached in _sc._cache.items():
                        if cached.source_url and cached.source_url.strip() == url_product:
                            shopify_pid = pid
                            break
                    if shopify_pid:
                        from shopify_client import delete_product as _del_shopify
                        deleted = await loop.run_in_executor(None, lambda: _del_shopify(shopify_pid))
                        if deleted:
                            await query.message.reply_text("🗑 Shopify product deleted.")
                except Exception as e:
                    logger.warning(f"[creative_hunt] Shopify delete failed: {e}")

            await query.message.reply_text(
                f"✅ <b>{_esc(back_sku)}</b> sent back to Pending.",
                parse_mode="HTML",
            )
            if session:
                await _advance_product(context.bot, session)
        else:
            await query.message.reply_text("❌ Failed to move product back to Pending.")
        return
    ...
    if data.startswith("ch_conf:"):
        ...
        ok = await asyncio.get_event_loop().run_in_executor(
            None, lambda: finalize_to_ready_for_ads(conf_sku, 10)
        )
    ...
    # ── DELETE CREATIVE ──────────────────────────────────────────────────────
    if data.startswith("ch_del_creative:"):
        dparts = data.split(":")
        if len(dparts) >= 3:
            del_sku = dparts[1]
            del_idx = int(dparts[2])
            ok = await asyncio.get_running_loop().run_in_executor(
                None, lambda s=del_sku, i=del_idx: delete_creative_slot(s, i, APPROVED_TAB)
            )
            if ok:
                await query.message.reply_text("🗑 Creative deleted.")
                session2 = _sessions.get(user_id)
                if session2:
                    products = await asyncio.get_running_loop().run_in_executor(
                        None, load_approved_products
                    )
                    session2["products"] = products
                    updated = next((p for p in products if p.get("SKU","").strip() == del_sku), None)
                    if updated and session2["product_idx"] < len(products):
                        session2["products"][session2["product_idx"]] = updated
                    await _show_product_prompt(context.bot, session2)
            else:
                await query.message.reply_text("❌ Could not delete creative. Please try again.")
        return
    ...
    if action == "ch_approve":
        ...
        # Save the Ads Library link (short, stable) instead of long CDN URLs.
        # ads_launch_bot → meta_ads_service.download_media() already resolves
        # Ads Library pages to CDN URLs at download time, so this is safe.
        ...
        ad_url = lib_url or media_url_cdn  # prefer library link, CDN as fallback
...
async def _bg_save_pending_product(
    ...
    # The user's Telegram chat_id is the same across all bots — just use the
    # research bot token to send to the same chat_id from the creative-hunt session.
    research_token = os.environ.get("TELEGRAM_BOT_TOKEN", "")
    if research_token:
        notify_bot     = TGBot(token=research_token)
        notify_chat_id = chat_id   # same Telegram user ID
    else:
        # Fallback: send via the creative-hunt-bot itself
        notify_bot     = bot
        notify_chat_id = chat_id
        logger.warning("[bg_save_pending] TELEGRAM_BOT_TOKEN not set — notifying in creative-hunt chat")
